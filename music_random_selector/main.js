...
$(document).ready(function(){ 
    $("input").click(function(){                 //按下button做input後執行
        let numberOfListItem = $("#music_type li").length;   //抓清單的長度
        //Math.randam():0~0.999999 
        //Math.randam():0~0.999999 *3 = 0~2.99999
        //Math.floor()會回傳小於等於所給數字的最大整數
        //要變成整數，才能在0~2之間取隨機變數
        let randomChildNumber = Math.floor(Math.random()*numberOfListItem); 
        $("#music_genres").text($("#music_type li").eq(randomChildNumber).text());   //抓出清單內相對應的文字
        $("#music_picture").attr("src", picture[randomChildNumber]);     //抓出清單內相對應的圖片
        $("#music_discrible").text(text[randomChildNumber]);
    })
})
